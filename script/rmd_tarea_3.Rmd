...
1.  ¿Qué interpretas de estas gráficas?#HE home environment entorno habitual dietas normales antes
...
existe una diferencia notoria entre las muestras segun la nacionalidad esto puede observarse especialmente en indice de simpson donde la dominancia de pocas especies es notablemente mayor.En cuento a la dieta,si bien se pueden observar cambios antes y despues de la intervencion estos no son tan evidentes como lo es la nacionalidad. 
...
ggplot(obj_f_filtrado, aes(x = SampleType, y = Abundance, fill = Phylum)) +
  ...
  dev.off()
```
existe una mayor abundancia genral en feces la que es mayor al doble en comparacion con skin y soli, los phylums más abundantes son firmicutes y bacteroidetes en feces, firmicutes y proteobacteria en skin y proteobacteria y verrucomicrobia en soil.
## Diversidad Beta
...
1.  ¿Qué ambiente muestra mayor diversidad alfa? ¿A qué factores podría
    deberse? el ambiente con mayor diversidad alfa es feces ya que tiene los valores más bajos en el indice de simpson en cambio el ambiente con menor diversidad es skin, esto puede deberse a que 
2.  ¿Qué se observa en la ordenación PCoA respecto a similitudes entre
    muestras? las muestras se ordenan naturalmente sin compartir espacios, lo que indica que los philums más dominantes en cada ambiente no estan presentes en todos ellos si no que cambian deacuerdo al ambiente
3.  ¿Qué patrón de dominancia taxonómica muestran las curvas de rango-abundancia? existe una dominanacia en general de firmicutes, aunque el segundo phylum cambia deacuerdo al ambiente.
4.  ¿Qué phyla son dominantes en cada ambiente y qué implicaciones
    ecológicas sugieren?
existe una mayor abundancia genral en feces la que es mayor al doble en comparacion con skin y soli, los phylums más abundantes son firmicutes y bacteroidetes en feces, firmicutes y proteobacteria en skin y proteobacteria y verrucomicrobia en soil.lo que sugiere que la riqueza puede ser parecida pero la abundancia varia en relacion a el ambiente. 